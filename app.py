...
def scan_experiments(base_dir):
    """Recursively scan base_dir for experiment folders."""
    results = []
    angle_re = re.compile(r"(\d+) Degree")
    temp_re = re.compile(r"(\d+)F")
    aer_trial_re = re.compile(r"([\d_]+) Percent Trial (\d+)")
    ...
    for angle_name in os.listdir(base_dir):
        ...
        if not os.path.isdir(angle_path):
            continue
        am = angle_re.fullmatch(angle_name)
        if not am:
            st.warning(f"Unexpected directory: {angle_name}")
            continue
        angle = int(am.group(1))
        for temp_name in os.listdir(angle_path):
            temp_path = os.path.join(angle_path, temp_name)
            tm = temp_re.fullmatch(temp_name)
            if not tm:
                st.warning(f"Unexpected directory: {temp_name}")
                continue
            temp = int(tm.group(1))
            for aer_trial_name in os.listdir(temp_path):
                aer_trial_path = os.path.join(temp_path, aer_trial_name)
                atm = aer_trial_re.fullmatch(aer_trial_name)
                if not atm:
                    st.warning(f"Unexpected directory: {aer_trial_name}")
                    continue
                ...
                for flow_name in os.listdir(aer_trial_path):
                    ...
                    try:
                        ...
                        st.warning(f"Unexpected directory: {flow_name}")
                        continue
                    results.append({
                        "path": flow_path,
                        "angle": angle,
